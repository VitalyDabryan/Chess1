public class ChessBoard {
    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (startLine == 0 && startColumn == 0) {
            this.board[0][0].check = false;
        } else if (startLine == 0 && startColumn == 4) {
            this.board[0][4].check = false;
        } else if (startLine == 0 && startColumn == 7) {
            this.board[0][7].check = false;
        }

        if (startLine == 7 && startColumn == 0) {
            this.board[7][0].check = false;
        } else if (startLine == 7 && startColumn == 4) {
            this.board[7][4].check = false;
        } else if (startLine == 7 && startColumn == 7) {
            this.board[7][7].check = false;
        }
...
//                    var10000.print(var10001 + this.board[i][j].getColor().substring(0, 1).toLowerCase() + "\t");
    ...
    public boolean canMoveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (this.nowPlayer.equals("White")) {
            if (this.board[endLine][endColumn] != null && this.board[endLine][endColumn].getColor().equals("Black")) {
                return true;
            } else {
                return this.board[endLine][endColumn] != null && this.board[endLine][endColumn].getColor().equals("White");
            }
        } else {
            return false;
        }
        ...
        if (this.nowPlayer.equals("White")) {
            if (this.board[0][7] != null && this.board[0][4] != null) {
                if (this.board[0][7].getSymbol().equals("R") && this.board[0][4].getSymbol().equals("K") && this.board[0][6] == null && this.board[0][5] == null) {
                    if (this.board[0][7].getColor().equals("White") && this.board[0][4].getColor().equals("White") && this.board[0][7].check && this.board[0][4].check && !(new King("White")).isUnderAttack(this, 0, 5)) {
                        this.board[0][4] = null;
